...
int main(){
  ...
  coral::Context::instance().loadComponent("COND/Services/TBufferBlobStreamingService",&pluginmanager);
  ...
  pool::IBlobStreamingService* blobstreamingService=coral::Context::instance().query<pool::IBlobStreamingService>().get();
  datasvc->configuration().setBlobStreamer(blobstreamingService, false);
